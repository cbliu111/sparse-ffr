...
from pp.neighbor import neighbors_precomputed
...
def _density_normalize(connectivities, matrix: Union[np.ndarray, sp.spmatrix]) -> Union[np.ndarray, sp.spmatrix]:
    """
    Density normalization by the underlying kNN graph.

    Parameters
    ----------
    matrix: object to normalize.
    ...
    q = np.asarray(connectivities.sum(axis=0)).squeeze()
...
def _compute_connectivity_tmat(
    ...
    conn = neighbors_precomputed(adata, mask=mask, n_neighbors=n_neighbors, metric_key=key)
    if density_normalize:
        return _density_normalize(conn, conn)
    else:
        return conn
