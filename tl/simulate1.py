...
def sim_single(params):
    ...
    # np.savetxt(f'./data/sim/num_tra_{i}.csv', num_tra, delimiter=",")
    # np.savetxt(f'./data/sim/total_Fx_{i}.csv', total_Fx, delimiter=",")
    # np.savetxt(f'./data/sim/total_Fy_{i}.csv', total_Fy, delimiter=",")
    # np.savetxt(f'./data/sim/path{i}.csv', np.stack([x_path, y_path]).T, delimiter=",")
    # np.savetxt('./data/sim/path_time' + str(i) + '.txt', list(zip(x_path, y_path)), fmt='%.5f %.5f',
...
def sim_parallel(
    ...
    results = Parallel(n_jobs=cpus)(
        delayed(sim_single)(params + tuple([i]))
